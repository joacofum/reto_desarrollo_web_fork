...
public class ListTaskService {
    ...
    public ListTaskModel completarTask(Long id) {
        ListTaskModel ltm = listTaskRepository.findById(id).get();
        ltm.setCompleted(!ltm.getCompleted());
        return listTaskRepository.save(ltm);
    }
}
